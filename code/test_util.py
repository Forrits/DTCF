...
def LA_test_all_case(vnet,resnet, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        # id = image_path.split('/')[-1]
        id = image_path.split('/')[0]
        h5f = h5py.File("dataset/LA_data/"+image_path, 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
...
def Pancer_test_all_case(vnet,resnet, image_list, num_classes, patch_size=(96,96,96), stride_xy=16, stride_z=16, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        
        id = image_path #用/分割后的列表的最后一个元素
        print("---------")
        print(id)
       
        h5f = h5py.File("dataset/pancreas_data/"+image_path, 'r')
...
def Bra_test_all_case(vnet,resnet, image_list, num_classes, patch_size=(96,96,96), stride_xy=16, stride_z=16, save_result=True, test_save_path=None, preproc_fn=None):
    total_metric = 0.0
    for image_path in tqdm(image_list):
        id = image_path #用/分割后的列表的最后一个元素
        print("---------")
        print(id)
        h5f = h5py.File("dataset/data/BraTS2019/"+image_path+'.h5', 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        if preproc_fn is not None:
            image = preproc_fn(image)
...
def test_single_case(vnet,resnet=None, image=None, stride_xy=None, stride_z=None, patch_size=None, num_classes=1):
 ...
 ################ DTCF双子网的测试
                if vnet is not None:
                    _,y1=vnet(test_patch)
                    #y1=vnet(test_patch)
                    y = F.softmax(y1, dim=1)
                    y = y.cpu().data.numpy()
                    y = y[0,:,:,:,:]
                if resnet is not None:
                    _,y2 = resnet(test_patch)
