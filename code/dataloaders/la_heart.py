...
class LAHeart(Dataset):      ##自定义数据集的类
    ...
    def __init__(self, base_dir=None, split='train',train_flod=None, common_transform=None,sp_transform=None):
        ...
        self.sp_transform = sp_transform                   #转换为tensor数据类型
        self.sample_list = []        
        print(train_flod)
        if split=='train':                                  
...
class RandomCrop(object):  #裁剪类
    """
    Crop randomly the image in a sample    随机裁剪样本中的图像
    Args:
...
class RandomRotFlip(object):   #随机旋转、反转类
    """
...
class RandomNoise(object):  #随机噪声类
...
class CreateOnehotLabel(object):        #创建one-hot编码
...
class GaussianBlur(object):    #高斯模糊
...
class TwoStreamBatchSampler(Sampler):
    ...
    def __len__(self):   #迭代器的长度，即是一次采样的数量
