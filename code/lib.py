...
from dataloaders.la_heart import RandomNoise

def transforms_for_noise(inputs_u2):
...
def transforms_for_scale(ema_inputs, image_size):
    ...
    for idx in range(ema_inputs.shape[0]):
        ...
        print("fafgg____{}".format(ema_inputs.shape[0]))
        img = np.transpose(ema_inputs[idx].cpu().numpy(), (1,0))
        # crop
